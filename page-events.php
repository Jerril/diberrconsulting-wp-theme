...
?>
      ...
      <div class="main-page-content">
        ...
        <?php 
            $contents = get_posts(array("post_type"=>"content", "post_status"=>"publish", "posts_per_page"=>-1, "meta_query"=>array(array("key"=>"content_type", "value"=>"event"))));
            ...
            if(!empty($type_query)){
              $event = get_term_by('slug', $type_query, 'event-category');

              foreach($contents as $i=> $content){
                $grp = $content->event;
                $grp = $grp['event_category'] ?? '';
                if($grp != $event->term_id){
                  unset($contents[$i]);
                }
              }
            }
              ...
              <div class="col-md-3 primaryborderright py-md-4 mt-4 mt-md-0">
                ...
                <div class="alignicons mt-1">
                  <select name="type" class="form-control select topselect secondaryinput">
                    ...
                    <?php $types = get_terms(array('taxonomy'=>'event-category', 'hide_empty'=>false));
                      if($types){
                        foreach($types as $type){ print_r($type);?>
