...
?>
      ...
      <div class="main-page-content">
        ...
        <?php 
            $contents = get_posts(array("post_type"=>"content", "post_status"=>"publish", "posts_per_page"=>-1, "meta_query"=>array(array("key"=>"content_type", "value"=>"resource"))));
            ...
            if(!empty($type_query)){
              $resource = get_term_by('slug', $type_query, 'resource');

              foreach($contents as $i=> $content){
                $grp = $content->download;
                $grp = $grp['download_category'] ?? '';
                if($grp != $resource->term_id){
                  unset($contents[$i]);
                }
              }
            }
              ...
              <div class="col-md-3 primaryborderright py-md-4 mt-4 mt-md-0">
                ...
                <div class="alignicons mt-1">
                  <select name="type" class="form-control select topselect secondaryinput">
                    ...
                    <?php $types = get_terms(array('taxonomy'=>'resource', 'hide_empty'=>false));
                      if($types){
