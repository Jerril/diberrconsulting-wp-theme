...
?>
      ...
      <div class="main-page-content">
        ...
        <?php 
            $contents = get_posts(array("post_type"=>"content", "post_status"=>"publish", "posts_per_page"=>-1, "meta_query"=>array(array("key"=>"content_type", "value"=>"value-proposition"))));
            ...
            if(!empty($type_query)){
              $vp = get_term_by('slug', $type_query, 'value-proposition');

              foreach($contents as $i=> $content){
                $grp = $content->value_proposition_group;
                $grp = $grp['vp_category'] ?? '';
                if($grp != $vp->term_id){
                  unset($contents[$i]);
                }
              ...
              <div class="col-md-3 primaryborderright py-md-4 mt-4 mt-md-0">
                ...
                <div class="alignicons mt-1">
                  <select name="type" class="form-control select topselect secondaryinput">
                    ...
                    <?php $types = get_terms(array('taxonomy'=>'value-proposition', 'hide_empty'=>false));
                      if($types){
